	...
	class Card : Panel {
		...
		public Card(MessageReceivedEventArgs e, Command dismissCommand) {
			...
			layout.Add(new Label {
				...
				Font = Fonts.Sans(9), TextColor = new Color(0, 0, 0, 0.8f) });
			if (priority != 3) layout.Add(new Label {
				Text = "(" + priority + ") ", Font = Fonts.Sans(9),
				TextColor = new Color(0, 0, 0, 0.8f) });
			layout.Add(new LinkButton {
				Text = "(dismiss)", Font = Fonts.Sans(9), Command = DismissCommand,
				TextColor = new Color(0, 0, 0, 0.8f) });
			...
			layout.BeginVertical();
			// title
			if (title != null) layout.AddRow(new Label {
				Text = title, Font = Fonts.Sans(9, FontStyle.Bold), Wrap = WrapMode.Word });
			// message
			if (message != null) layout.AddRow(new Label { Text = message, Wrap = WrapMode.Word });
			layout.EndVertical();

			Content = new Panel{
				BackgroundColor = new Color(0, 0, 0, .05f),
				Content = layout,
			};
		}
	}
